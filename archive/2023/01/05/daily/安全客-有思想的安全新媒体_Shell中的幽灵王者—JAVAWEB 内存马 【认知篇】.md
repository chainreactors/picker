...
url: https://www.anquanke.com/post/id/284235
source: 安全客-有思想的安全新媒体
date: 2023-01-05
fetch_date: 2025-10-04T03:02:34.456666
---
...
这样的概念在安全行业里一直没有得到完全的重视，只有一些做研究的师傅在社区交流和探索相关的实现，直到冰蝎 Behinder\_v3.0 Beta 7 的更新将 java agent 类型的内存马进行了实现，将此种技术带入大众视野。
...
> LandGrey 于 2020 年发布了 copagent 项目，基于 Java Agent
> 技术查找关键类的子类、关键注解类，并通过恶意包名标识类，随后将这些类 dump 出来并匹配其中的风险关键字。
>
> c0ny1 也发布了 java-memshell-scanner 项目，基于 Tomcat API
> 查找关键位置中的信息（Servlet/Filter），通过 ClassLoader
> 判断是否在磁盘上、类名（包名）等信息进行内存马的检出，还支持将 Class 从内存中 dump 出来进行人工分析。
>
...
> 冰蝎通过删除 /tmp/.java\_pid+{pid} 文件来阻止 JVM 进程通信，从而禁止查杀 Agent 加载。
>
> ZhouYu 通过阻止后续的 ClassFileTransformer 加载的方式阻止其他 Java Agent 加载来防止检测和查杀。
>
> Glassy 师傅分享通过 Bootstrap ClassLoader 加载恶意类来绕过对内存马的检测。
>
> 冰蝎及哥斯拉作者还针对 Agent Self Attach 以及无文件落地注入 Agent 内存马进行了技术上的突破，可以无文件落地注入
> agent 内存马。
...
<https://v.qq.com/x/page/n3366gmdxgn.html>
...
1、2017 – [Tomcat 源代码调试笔记 – 看不见的 Shell](https://mp.weixin.qq.com/s/x4pxmeqC1DvRi9AdxZ-0Lw)
...
8、2022 – [RASP攻防下的黑魔法](https://mp.weixin.qq.com/s/ab5GtASzS_UuaILp2vAEdw)
