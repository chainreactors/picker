---
title: .NET 一款内置9种不同混淆方法的工具
url: https://mp.weixin.qq.com/s?__biz=MzUyOTc3NTQ5MA==&mid=2247496025&idx=1&sn=fc31dac42342eb9349878d70d34e2157&chksm=fa595fb4cd2ed6a2e555c76b7fc6ec9695cb1870409fe505f1f77fbb9d71d727cf05eaa3a893&scene=58&subscene=0#rd
source: dotNet安全矩阵
date: 2024-10-17
fetch_date: 2025-10-06T18:55:50.906978
---

# .NET 一款内置9种不同混淆方法的工具

![cover_image](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1Yib5CJlFmEOTeruxSS2L8gEiaFicHLxbzFIHic7vluAFTMLQfm1hPdE4RDSWG0mDWicqqDkD5ruEx9XZ5A/0?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1Yib5CJlFmEOTeruxSS2L8gEiajJSdqjFqj2fVFicgk7OyNjBQ2SVMbW2k5j2P48EMzSVgygicUYKyoaxw/640?wx_fmt=jpeg&from=appmsg)
...
```
1.JunkProtection：通过插入无用代码来增加代码的冗余性，干扰分析人员的理解。
2.NumbersToModuleReference：将数值转化为模块引用，增加逆向工程的难度。
3.MutationObf：使用代码变异，使代码的原始逻辑更加难以推测。
```
...
```
```
Sharp4StrongObfuscator.exe -f <TargetFile> -o <Option>
```
```
...
```
```
-o 8：表示使用 JunkProtection 混淆方法。
-o 7：表示使用 NumbersToModuleReference 混淆方法。
-o 6：表示使用 MutationObf 混淆方法。
```
```
...
```
```
Sharp4StrongObfuscator.exe -f StartInSelectedDesktop.exe -o 8
Sharp4StrongObfuscator.exe -f StartInSelectedDesktop.exe -o 6
```
```

上述命令会根据参数 -o 所指定的混淆方法，对目标文件进行不同类型的混淆，生成处理后的程序集文件StartInSelectedDesktop.exe.obfuscated。

![](https://mmbiz.qpic.cn/mmbiz_png/NO8Q9ApS1Yib5CJlFmEOTeruxSS2L8gEiavyy51qPsYZJW4Kfg92BweSSzic8bzibDr7ibRMfQBsXgGYaXT7UFBIPxg/640?wx_fmt=png&from=appmsg)

改成StartInSelectedDesktop7.exe，通过dnSpy打开反编译看出函数名均已混淆，变得不可阅读。

![](https://mmbiz.qpic.cn/mmbiz_png/NO8Q9ApS1Yib5CJlFmEOTeruxSS2L8gEiarjic27xlVyze323oH4M7X7C2hEwuna9uL5ttsAic4BusKbnt3w2ndwyA/640?wx_fmt=png&from=appmsg)

测试运行结果，发现一切正常，StartInSelectedDesktop7.exe default "cmd.exe" 成功弹出新的cmd窗口。

综上，Sharp4StrongObfuscator 是一款功能强大的 .NET 程序混淆工具，通过简单的命令即可为程序集提供多种混淆方案。工具已经打包在星球，感兴趣的朋友可以加入自取。
