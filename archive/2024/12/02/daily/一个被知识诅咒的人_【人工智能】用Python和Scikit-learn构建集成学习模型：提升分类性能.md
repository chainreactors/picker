---
title: 【人工智能】用Python和Scikit-learn构建集成学习模型：提升分类性能
url: https://blog.csdn.net/nokiaguy/article/details/144171910
source: 一个被知识诅咒的人
date: 2024-12-02
fetch_date: 2025-10-06T19:35:47.501114
---
...
[#scikit-learn](https://so.csdn.net/so/search/s.do?q=scikit-learn&t=all&o=vip&s=&l=&f=&viparticle=&from_tracking_code=tag_word&from_code=app_blog_art)
...
设有 (n) 个样本，模型数为 (M)，第 (i) 个样本的预测为 (f\_i(x))。Bagging 的预测公式为：
  F ( x ) = 1 M ∑ i = 1 M f i ( x ) F(x) = \frac{1}{M} \sum\_{i=1}^M f\_i(x) F(x)=M1​i=1∑M​fi​(x)

随机森林是 Bagging 的一种改进，通过引入特征随机性，进一步减少模型的相关性。

---

#### 2.2 Boosting（提升）

Boosting 是一种逐步构建模型的过程，每个新模型旨在减少前一轮模型的误差。

##### 梯度提升的核心思想

梯度提升通过最小化损失函数 (L) 来更新模型参数。其每一步的更新公式为：
