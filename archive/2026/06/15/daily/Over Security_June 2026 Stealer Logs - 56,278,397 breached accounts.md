---
title: June 2026 Stealer Logs - 56,278,397 breached accounts
url: https://haveibeenpwned.com/Breach/June2026StealerLogs
source: Over Security
date: 2026-06-15
fetch_date: 2026-06-16T07:16:38.517624
---

# June 2026 Stealer Logs - 56,278,397 breached accounts

[![Have I Been Pwned](/Images/HibpLogo.svg?v=w1UPiLz7TzzzyTflCj6n2lzgzKj-UhDA7827u2P4wb8)](/)

[Dashboard](/Dashboard)

* [Who's Been Pwned](/PwnedWebsites)
* [Passwords](/Passwords)
* [Notify Me](/NotifyMe)
* [API](/API/v3)
* [Demos](/Demos)
* [Pricing](/Subscription)
* About
  + [Who, What & Why](/About)
  + [FAQs](/FAQs)
  + [Terms of Use](/TermsOfUse)
  + [Privacy Policy](/Privacy)
  + [Opt Out](/OptOut)

[Dashboard](/Dashboard)

![June 2026 Stealer Logs Logo](https://logos.haveibeenpwned.com/List.png)

# June 2026 Stealer Logs Data Breach

[StealerLogs Breach](/FAQs#StealerLogs)

## What Happened

In June 2026, a collection of accumulated stealer logs from various sources was added to HIBP. The corpus comprised 56M unique email addresses across hundreds of millions of stealer log records. The data also contained 124M unique passwords, which have been added to [Pwned Passwords](https://haveibeenpwned.com/Passwords) and are now searchable. Individuals can view any records captured against their email address in [the stealer logs section of their dashboard](https://haveibeenpwned.com/Dashboard/Redirect/StealerLogs). Organisations can see logs affecting their domain via [the stealer logs API](https://haveibeenpwned.com/API/v3#StealerLogsOverview).

## Compromised Data

Email addresses

Passwords

## Recommended Actions

##### Change Your Password

If you haven’t already changed the password affected by this breach, do
so immediately on every account where it was used.

##### Enable Two-Factor Authentication

Wherever 2FA is supported, add an extra layer of security to your
account.

Sponsored

![1Password](/Images/PartnerLogos/1Password.svg)

Use a password manager to generate and store strong, unique passwords for all your accounts. 1Password helps protect your data with industry-leading security.

[Try 1Password](https://1password.com/haveibeenpwned)

Sponsored

![Guardio](/Images/PartnerLogos/Guardio.svg)

Get Guardio for real-time protection after a breach. Guardio blocks AI-generated scam sites, fake login pages, and malicious pages designed to exploit leaked information. Built by cybersecurity specialists who track new threats 24/7, Guardio gives you immediate, expert-level protection plus clear steps to help you secure your accounts instantly.

[Try Guardio](https://guard.io/hibpad)

Sponsored

![Aura](/Images/PartnerLogos/Aura.svg)

Get Aura for identity theft and credit protection. Keep your assets safe with fast fraud alerts, instant credit lock, and $1,000,000 identity theft insurance. Speak to a U.S. based fraud specialist 24/7.

[Try Aura](https://buy.aura.com/hibp)

### Breach Overview

* Affected Accounts:

  56.3 million
* Breach Occurred:

  June 2026
* Added to HIBP:

  15 Jun 2026

## Recommended Actions

##### Change Your Password

If you haven’t already changed the password affected by this breach, do so
immediately on every account where it was used.

##### Enable Two-Factor Authentication

Wherever 2FA is supported, add an extra layer of security to your account.

Sponsored

![Guardio](/Images/PartnerLogos/Guardio.svg)

Get Guardio for real-time protection after a breach. Guardio blocks AI-generated scam sites, fake login pages, and malicious pages designed to exploit leaked information. Built by cybersecurity specialists who track new threats 24/7, Guardio gives you immediate, expert-level protection plus clear steps to help you secure your accounts instantly.

[Try Guardio](https://guard.io/hibpad)

Sponsored

![Aura](/Images/PartnerLogos/Aura.svg)

Get Aura for identity theft and credit protection. Keep your assets safe with fast fraud alerts, instant credit lock, and $1,000,000 identity theft insurance. Speak to a U.S. based fraud specialist 24/7.

[Try Aura](https://buy.aura.com/hibp)

Sponsored

![1Password](/Images/PartnerLogos/1Password.svg)

Use a password manager to generate and store strong, unique passwords for all your accounts. 1Password helps protect your data with industry-leading security.

[Try 1Password](https://1password.com/haveibeenpwned)

[Have I Been Pwned](/)

Check if your email has been compromised in a data breach

[Check my email](/)

##### Services

* [Email Search](/)
* [Domain Search](/Dashboard#Domains)
* [Who's Been Pwned](/PwnedWebsites)
* [Passwords](/Passwords)
* [Notify Me](/NotifyMe)
* [API](/API/v3)
* [MCP Server](/Docs/MCP)
* [Pricing](/Subscription)
* [Opt Out](/OptOut)
* [Dashboard](/Dashboard)
* [Partners](/Partners)

##### Information

* [Demos](/Demos)
* [Who, What & Why](/About)
* [FAQ](/FAQs)
* [Privacy Policy](/Privacy)
* [Terms of Use](/TermsOfUse)
* [DPA](/DPA)
* [Suggest a Feature](https://haveibeenpwned.uservoice.com/)
* [Donate](/Donate)
* [Merch](https://merch.haveibeenpwned.com/)

##### Connect With Us

© 2026 Have I Been Pwned®. All rights reserved.

The content on this page was tailored for United States