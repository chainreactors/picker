---
title: 安天AVL SDK反病毒引擎升级通告（20260502）
url: https://mp.weixin.qq.com/s/oAE3xBfirPm7Bu6COe_X2Q
source: Doonsec's feed
date: 2026-05-02
fetch_date: 2026-05-03T05:26:35.945066
---
...
统计周期：2026年04月25日~2026年05月01日

安天AVL SDK反病毒引擎本周共发布病毒库更新84次，日均更新12次，新增可检测恶意代码家族39个，新增可检测恶意代码变种6,987个，新增检测规则23,813条。
...
| --- | --- | --- |
| **序号** | **病毒名** | **病毒描述** |
| 1 | Trojan/Win64.GenieLocker[Ransom] | 该家族是一种木马病毒，通常通过钓鱼邮件、恶意下载或漏洞利用等方式传播。一旦感染，该恶意软件会加密用户的文件，并要求支付赎金以获取解密密钥，对计算机系统造成严重威胁。 |
| 2 | Trojan/Linux.PoisonedRefresh | 该家族是一种木马病毒，通常伪装成合法软件或通过软件漏洞、恶意下载等途径传播。它的主要目标是窃取敏感信息、建立持久后门，并可能参与僵尸网络活动，对系统安全构成严重威胁。 |
| 3 | Trojan/Script.Maxishope | 该家族是一种木马病毒，主要通过恶意脚本（如VBScript或JavaScript）进行传播，一旦感染，会从远程服务器下载并执行更多恶意载荷，包括勒索软件、间谍软件或其他有害程序，从而对系统安全构成严重威胁。 |
| 4 | Trojan/MSIL.DeskVB[Spy] | 该家族是一种木马病毒，主要通过木马程序的方式潜伏在计算机系统中，监视用户的键盘输入和屏幕操作，记录敏感信息如账号、密码等。 |
| 5 | Trojan/Win64.SeaMonkey[Backdoor] | 该家族是一种木马病毒，通过欺骗用户或携带其他恶意软件来进入受害者的计算机系统。一旦感染，会在后台运行并以隐藏的方式窃取敏感信息，监视用户的活动并向远程服务器发送数据。 |
...
截至2026年05月01日24:00，AVL SDK反病毒引擎可检出分布在8个基础分类，57,793个恶意代码家族的18,559,197个恶意代码变种、总检测规则数41,865,379条。
...
| 感染式病毒 | 59,614 | 6,808,686 |
| 蠕虫 | 314,879 | 3,939,883 |
| 木马 | 13,506,012 | 25,503,214 |
| 黑客工具 | 457,542 | 362,774 |
| 风险工具 | 1,205,949 | 2,216,678 |
| 流氓软件 | 3,015,164 | 3,001,807 |
| 垃圾文件 | 11 | 1,079 |
| 测试程序（自检用） | 26 | 100 |
| 合计 | 18,559,197 | 41,865,379 |
...
近一周监测到游蛇（银狐）黑产组织活动主要通过仿冒网站进行传播，仿冒正规软件官网（如WPS、搜狗输入法、谷歌浏览器）等。

其主要攻击手法为：

1、使用Inno Setup安装包捆绑恶意程序，释放“白+黑”文件诱导分析人员，通过“白+黑”技术加载恶意组件。

2、写入注册表项，达成恶意程序持久化驻留目的。

3、从IDAT块提取隐写载荷，于内存中解密并运行远控木马（RAT）。

4、部署远控木马，具备持久化、远程控制、规避检测、终止安全软件及反虚拟机分析能力等恶意功能。
...
[安天AVL SDK反病毒引擎升级通告（20260411）](https://mp.weixin.qq.com/s?__biz=MjM5MTA3Nzk4MQ==&mid=2650214463&idx=1&sn=46cbeaa3e3e82f03780ffbab7db5baf7&scene=21#wechat_redirect)
