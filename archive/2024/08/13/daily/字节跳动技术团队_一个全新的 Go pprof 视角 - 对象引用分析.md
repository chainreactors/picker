---
title: 一个全新的 Go pprof 视角 - 对象引用分析
url: https://mp.weixin.qq.com/s?__biz=MzI1MzYzMjE0MQ==&mid=2247508729&idx=1&sn=0189ca7ee4cd24aade8acedda4e3e400&chksm=e9d3691bdea4e00d742dd30ce32b690dc5cd9e87296e2d8ba15e1256bccbf1736c0ee0af01c5&scene=58&subscene=0#rd
source: 字节跳动技术团队
date: 2024-08-13
fetch_date: 2025-10-06T18:07:42.006668
---

# 一个全新的 Go pprof 视角 - 对象引用分析

![cover_image](https://mmbiz.qpic.cn/sz_mmbiz_jpg/5EcwYhllQOgBSUFRQLAAhS5po8jqkn1JGnjz0sQR0261uxYtCKfic7Rq4fQESic7Uia7XbD0GznBvtKpuXUzzNEJg/0?wx_fmt=jpeg)
...
在Go语言开发中，内存泄漏问题往往难以定位，传统的Pprof工具虽然能提供一定帮助，但在复杂场景下其能力有限。为了更高效地分析和解决这些问题，CloudWeGo 团队开发了一款新的工具——Goref。Goref 基于 Delve，能够深入分析Go程序的堆对象引用，显示内存引用的分布，帮助开发者快速定位内存泄漏或优化GC开销。该工具不仅支持运行时进程的分析，还能分析核心转储文件，为 Go 开发者提供了一个强大的内存分析工具。项目已开源在 GitHub 上，欢迎社区贡献和使用。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/BvYvvlJIDbKibEapl1SBMaDica0Eesz4WeNLwv9wiaoYfeWSyw5VJdic4yNW9HfPkUrzMGSKKbOibsW6jMUibFNiaCeRQ/640?wx_fmt=other&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/BvYvvlJIDbKibEapl1SBMaDica0Eesz4WeyraAUm6VerMxu300MAicsD7qLNOFfib5SvxnibiaF3fkLoUGW6QsFEd9Dw/640?wx_fmt=other&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
1. 1. 其中，Type 属性记录了它的类型信息，按 DWARF 格式递归遍历，可以进一步确定变量的每一个子对象类型；
2. 2. Location 则是一个相对复杂的属性，它记录了一个可执行的表达式或者一个简单的变量地址，作用是确定一个变量的内存地址，或者返回寄存器的值。在全局变量解析时，Delve 通过它获得了变量的内存地址。
...
1. 1. 待分析进程的各个 Goroutine stack 的地址空间范围，并包括每个 Goroutine stack 存储 gcmask 的 `stackmap`，用来标记是否可能指向一个存活的堆对象；
2. 2. 待分析进程的各个 data/bss segment 的地址空间范围，包括每个 segment 的 gcmask，也是用来标记是否可能指向一个存活的堆对象；
3. 3. 以上两步都是获取 GC Roots 的必要信息；
4. 4. 最后一步是读取待分析进程的 `mspan` 索引，以及每个 `mspan` 的 base、elem size、gcmask等信息，在工具的内存中复原这个索引；
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/BvYvvlJIDbKibEapl1SBMaDica0Eesz4We9sVBnUCTiaYDL4NwOT2ia4ciaDSAl7thvK99FrB8hGGOIAn5Z8gKOgC4g/640?wx_fmt=other&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/BvYvvlJIDbKibEapl1SBMaDica0Eesz4We5giaYkXrmHORibKYc1jNRLpowLgwtribwuwLzYK3fvxXbhqdUAwhvBVOQ/640?wx_fmt=other&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

图中展示了每个 root 变量的名称，以及其引用的字段名和类型名。注：由于 Go1.23 之前 DWARF Info 没有支持闭包类型的字段 offset，所以闭包变量`wpool.(*Pool).GoCtx.func1.task`暂时无法展示下游对象。

选择 `inuse_objects` 标签，还可以查看对象数分布火焰图：

![](https://mmbiz.qpic.cn/sz_mmbiz_png/BvYvvlJIDbKibEapl1SBMaDica0Eesz4WeEEUALjzCwiasxNUhN8ynfDkn37eqngKNqhhyC2xgvQQ7tbtvkPLzicpg/640?wx_fmt=other&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

项目地址：https://github.com/cloudwego/goref，点击“阅读原文”即可直达。
