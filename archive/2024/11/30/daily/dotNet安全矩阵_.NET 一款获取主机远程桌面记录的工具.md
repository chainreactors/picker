---
title: .NET 一款获取主机远程桌面记录的工具
url: https://mp.weixin.qq.com/s?__biz=MzUyOTc3NTQ5MA==&mid=2247497088&idx=1&sn=d311faf22562f142c5795c79ca6e7bb4&chksm=fa595b6dcd2ed27b48111424379a781810c66a5f8e508c1e027b87cb129d5cb077f30a80b13a&scene=58&subscene=0#rd
source: dotNet安全矩阵
date: 2024-11-30
fetch_date: 2025-10-06T19:16:39.133972
---

# .NET 一款获取主机远程桌面记录的工具

![cover_image](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1Y9QTJKo9MQE7Fjk8MCxnWJq0DAzV4QvW4sSZqfLiclGVIVSxVo3F3YfBVtMbhPe5PbP33bLA1Jooibw/0?wx_fmt=jpeg)
...
Sharp4RDPData.exe 是一款专门用于获取目标主机通过 mstsc（Microsoft Terminal Services Client） 进行远程桌面连接历史数据的工具。

![](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1Y9J3juCcnsnvw9dpicRqlmuFZSxvzLSTzIbYaYsMsGibwiam1oCZv0WQh0K7VWAibUZULG8VNvvUKzYmw/640?wx_fmt=jpeg&from=appmsg)

03

使用方法

该工具能够轻松提取目标主机上的远程桌面协议（RDP）外连和内连的详细信息，包括登录成功与失败的记录，帮助使用者全面了解目标主机的远程访问情况。在目标主机的命令行（CMD）下直接运行以下命令：
...
工具运行后，会自动扫描主机注册表并提取 RDP 连接相关的历史数据，结果以清晰的格式呈现。

![](https://mmbiz.qpic.cn/mmbiz_png/NO8Q9ApS1Y9QTJKo9MQE7Fjk8MCxnWJqO6pDGyeAdrJUlqs3iacUp9baHg8xheDFoN0tCW1F4gUDnPoVIuprjWA/640?wx_fmt=png&from=appmsg)

## 3.1 外连数据

RDP外连数据展示了目标主机曾通过 mstsc 客户端远程连接到其他主机的历史记录，包括远程主机的 IP 地址或主机名、用户账户等信息。

```
```
S-1-5-21-41947675454-204547724-3899999-500:
192.168.1.141 PC-20230\Ivan1ee
192.168.1.104 PC-20201\ivan1ee
192.18.30.26    ivan1ee
192.18.30.12    PC-20201112PLZR\ivan1ee
192.18.30.10    ivan1ee
```
```

用户的安全标识符（Security Identifier），对应的是发起 RDP 外连操作的本地主机用户。

## 3.2 内连数据

RDP内连数据记录了目标主机被其他主机通过 RDP 连接的历史情况，进一步细分为登录成功和登录失败的记录。

```
```
Login Successful:
2023-09-30T01:30:11  37 192.168.1.141 PC-202308\Administrator
2023-09-30T01:21:05  1  192.168.1.141 PC-202308\ivan1ee
```
```

Sharp4RDPData.exe 是一款强大且便捷的 RDP 数据提取工具，能够快速分析目标主机的远程连接历史记录，为安全分析和渗透测试提供重要依据。文章涉及的工具已打包在星球，感兴趣的朋友可以加入自取。
