---
title: .NET 一款通过核心进程转储凭据的工具
url: https://mp.weixin.qq.com/s?__biz=MzUyOTc3NTQ5MA==&mid=2247495485&idx=1&sn=cb9becc062b84c7364ef46025cc317c6&chksm=fa5941d0cd2ec8c6bc9206e55a56125b3cc3a768b2f7c85dde6544ec685127cbac037f8993b9&scene=58&subscene=0#rd
source: dotNet安全矩阵
date: 2024-09-25
fetch_date: 2025-10-06T18:27:05.560707
---

# .NET 一款通过核心进程转储凭据的工具

![cover_image](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1YibfAdnicNVezlJNraV6JJUwADlWZnYeFdDWCCDwu4Bp5gFcobcib49YL9Fmbtdg375FeulQ2fEHNiaBA/0?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1YibfAdnicNVezlJNraV6JJUwAAEu2WnLAo0Y6VTBg7DOa8muNSd2oWwZiapsbVg7Uq5EjuyaafydhURA/640?wx_fmt=jpeg&from=appmsg)

03

使用方法

Sharp4LsassDump的使用非常简单，支持两种运行方式：默认情况下，Sharp4LsassDump会在当前目录下生成转储文件。另外，可以传入一个参数[DUMP\_FILE]来指定转储文件的保存路径。具体用法如下所示。
...
首先，通过NtOpenProcess函数打开目标进程句柄，指定的权限为PROCESS\_QUERY\_INFORMATION和PROCESS\_VM\_READ，允许我们查询进程信息及读取其虚拟内存。接着，通过CustomGetModuleHandle函数获取lsasrv.dll模块的加载地址：
...
随后，将读取的数据与现有的memory\_regions数组拼接在一起，形成最终的转储数据
...
综上，Sharp4LsassDump是一款功能强大且隐蔽性高的lsass进程转储工具，适用于各种渗透测试场景。它通过NTAPI的直接调用来实现转储操作，避免了使用传统的MinidumpWriteDump函数，从而能够绕过多种防护措施。这款工具在Windows 10和11上的测试表现非常稳定，对于安全研究人员和渗透测试人员来说，具有较高的实用价值。工具已经打包在星球，感兴趣的朋友可以加入自取。
