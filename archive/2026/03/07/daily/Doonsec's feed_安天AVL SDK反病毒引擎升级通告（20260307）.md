---
title: 安天AVL SDK反病毒引擎升级通告（20260307）
url: https://mp.weixin.qq.com/s/jq0Y_Vbl2PjQPQY-f-Tyzg
source: Doonsec's feed
date: 2026-03-07
fetch_date: 2026-03-08T04:04:11.385997
---
...
统计周期：2026年02月28日~2026年03月06日

安天AVL SDK反病毒引擎本周共发布病毒库更新84次，日均更新12次，新增可检测恶意代码家族40个，新增可检测恶意代码变种7,931个，新增检测规则16,154条。
...
| --- | --- | --- |
| **序号** | **病毒名** | **病毒描述** |
| 1 | Trojan/Win64.MalaRansom[Ransom] | 该家族是一种木马病毒，通常通过钓鱼邮件、恶意下载或漏洞利用等方式传播。一旦感染，该恶意软件会加密用户的文件，并要求支付赎金以获取解密密钥，对计算机系统造成严重威胁。 |
| 2 | Trojan/Win32.JosEncrypt | 该家族是一种木马病毒，主要通过捆绑在看似合法的软件、钓鱼邮件附件或恶意网站链接中传播，主要目的是渗透用户系统，窃取敏感信息（如银行账户、密码、个人身份信息）并建立后门以供远程攻击者控制。 |
| 3 | Trojan/Win32.HarmonyLoader | 该家族是一种木马病毒，主要通过软件捆绑、钓鱼邮件或恶意网站等方式传播，具备较强的隐藏性和系统穿透能力。主要目的是建立远程控制通道，窃取用户敏感信息（如账户密码、财务数据、隐私文件等），并可能下载其他恶意模块。 |
| 4 | Trojan/VBS.FakeNV | 该家族是一种木马病毒，通常伪装成无害的系统文件或用户文档，通过电子邮件附件、恶意网站或捆绑软件进行传播。该病毒的主要目的是潜伏在受感染的计算机系统中，窃取用户的敏感信息，如登录凭证、银行账户信息和各类隐私数据。 |
| 5 | Trojan/Win32.Cripack | 该家族是一种木马病毒，主要通过钓鱼邮件、恶意下载链接或软件携带等方式传播，在感染用户计算机后会隐藏自身并窃取用户敏感信息，如账号密码、银行卡信息等，从而危害用户隐私安全。 |
...
截至2026年03月06日24:00，AVL SDK反病毒引擎可检出分布在8个基础分类，57,536个恶意代码家族的18,505,185个恶意代码变种、总检测规则数41,674,563条。
...
| 感染式病毒 | 59,556 | 6,801,899 |
| 蠕虫 | 314,287 | 3,935,617 |
| 木马 | 13,456,687 | 25,381,347 |
| 黑客工具 | 456,362 | 360,502 |
| 风险工具 | 1,204,712 | 2,204,459 |
| 流氓软件 | 3,013,544 | 2,989,562 |
| 垃圾文件 | 11 | 1,078 |
| 测试程序（自检用） | 26 | 99 |
| 合计 | 18,505,185 | 41,674,563 |
...
近一周监测到[游蛇（银狐）黑产组织](https://mp.weixin.qq.com/s?__biz=MjM5MTA3Nzk4MQ==&mid=2650213282&idx=1&sn=ccbd72c32a590fd553420e79cdda4502&scene=21#wechat_redirect)活动主要通过仿冒网站进行传播，仿冒正规软件官网如搜狗输入法、WPS、谷歌浏览器等。

其主要攻击手法为：

1、使用MSI安装包捆绑恶意程序，释放多组“白+黑”文件诱导分析人员，通过“白+黑”技术加载恶意组件。

2、创建计划任务或修改注册表实现持久化。

3、部署Winos 4.0远控木马，具备进程注入、远程控制能力，可终止多款安全软件。
