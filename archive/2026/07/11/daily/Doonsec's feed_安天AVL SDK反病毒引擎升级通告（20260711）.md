---
title: 安天AVL SDK反病毒引擎升级通告（20260711）
url: https://mp.weixin.qq.com/s/eKyPp0RltqTwA8-HYbCUOw
source: Doonsec's feed
date: 2026-07-11
fetch_date: 2026-07-12T05:08:57.660801
---

# 安天AVL SDK反病毒引擎升级通告（20260711）
...
统计周期：2026年07月04日~2026年07月10日

安天AVL SDK反病毒引擎本周共发布病毒库更新84次，日均更新12次，新增可检测恶意代码家族22个，新增可检测恶意代码变种5,674个，新增检测规则30,205条。
...
| --- | --- | --- |
| **序号** | **病毒名** | **病毒描述** |
| 1 | Trojan/Python.LoneNone | 该家族是一种木马病毒，主要通过植入恶意脚本来控制受感染主机，使攻击者能够远程执行恶意操作，窃取敏感数据，监视用户活动等。 |
| 2 | Trojan/Win32.TaxShadow | 该家族是一种木马病毒，主要通过木马程序的方式潜伏在计算机系统中，监视用户的键盘输入和屏幕操作，记录敏感信息如账号、密码等。 |
| 3 | Trojan/MSIL.RubbrCrypt[Ransom] | 该家族是一种木马病毒，通常通过钓鱼邮件、恶意下载或漏洞利用等方式传播。一旦感染，该恶意软件会加密用户的文件，并要求支付赎金以获取解密密钥，对计算机系统造成严重威胁。 |
| 4 | Trojan/MSIL.TinyRCT[Downloader] | 该家族是一种木马病毒，主要利用系统漏洞进行传播和感染，该病毒通过伪装成正常软件或隐藏在破解程序、盗版软件中诱骗用户下载执行，窃取用户的敏感信息，包括银行账户、密码、个人身份信息等重要数据。 |
...
截至2026年07月10日24:00，AVL SDK反病毒引擎可检出分布在8个基础分类，58,042个恶意代码家族的18,621,037个恶意代码变种、总检测规则数42,236,609条。
...
| 感染式病毒 | 59,670 | 6,829,206 |
| 蠕虫 | 315,632 | 3,952,045 |
| 木马 | 13,563,590 | 25,811,060 |
| 黑客工具 | 458,869 | 367,341 |
| 风险工具 | 1,207,131 | 2,248,607 |
| 流氓软件 | 3,016,107 | 3,027,163 |
| 垃圾文件 | 11 | 1,082 |
| 测试程序（自检用） | 27 | 105 |
| 合计 | 18,621,037 | 42,236,609 |
...
本周持续警惕AI开源供应链投毒安全风险。研究人员发现一组与Lazarus关联的恶意npm包，恶意家族Trojan/JS.NpmSteal[PSW]伪装成Rollup polyfill工具，通过仿冒名称、README和仓库信息降低开发者警惕。其攻击链分层设计明显：入口包在导入时静默安装第二阶段组件，再从远程服务拉取并执行混淆载荷。最终功能不仅包括窃取浏览器凭据、钱包数据和敏感文件，还具备远程控制、剪贴板监控、命令执行等能力。
