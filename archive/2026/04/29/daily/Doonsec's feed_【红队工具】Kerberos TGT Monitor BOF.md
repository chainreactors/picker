---
title: 【红队工具】Kerberos TGT Monitor BOF
url: https://mp.weixin.qq.com/s/E3yEN4vu9t4mZ-A_hgei1g
source: Doonsec's feed
date: 2026-04-29
fetch_date: 2026-04-30T05:28:01.126605
---

# 【红队工具】Kerberos TGT Monitor BOF

![cover_image](https://mmbiz.qpic.cn/mmbiz_jpg/EYGYnyEdzQWSD6PRkyFDgCicJRGt54gaPYiaiawIA0AR65jTdnf2EiaQwlZk6fiaZkUU4I009icV68tstmyTg2x0ibk8wmL5otLOmNJtUiaDhCQTWrU/0?wx_fmt=jpeg)
...
异步信标对象文件（BOF），监控Kerberos登录事件，并在捕获新的Kerberos TGT时唤醒代理。类似于Rubeus命令，该BOF无限运行，并定期检查系统上的LSA工单缓存。当检测到新的TGT时，打印工单元数据并输出base64编码的kirbi blob，可用于通过传递工单攻击进行横向移动。

```
Usage: tgt-monitor [--interval interval] [--user user]Example: tgt-monitor --interval 5 --user DC01$
Optional arguments:  --interval interval       INT        Polling interval in seconds (default: 60).  --user user               STRING     Target specific username only.
```

![](https://mmbiz.qpic.cn/sz_mmbiz_png/EYGYnyEdzQUvdibpIcfuZgpDZ72GvrHum43S5NBUjdibIFnd4NuericibFCLyOYw3GLiaia9Nb6eWib0FhZkpib1NAflgia0ThicZY5vuPaOoAdjeKAT0/640?wx_fmt=png&from=appmsg)
