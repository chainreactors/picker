---
title: libsignal v0.102.2
url: https://kitploit.com/en/posts/github-signalapp-libsignal-v01022
source: Kitploit
date: 2026-09-12
fetch_date: 2026-09-13T07:01:29.672473
---
...
![](https://assets.kitploit.com/production/public/tools/49934/44e536b9cd24c23be1711719a5a71d1b920024e83f99f49b10b0dee7ddd4ee9d.png)
...
* libsignal-protocol: Implements the Signal protocol, including the [Double Ratchet algorithm](https://signal.org/docs/). A
  replacement for [libsignal-protocol-java](https://github.com/signalapp/libsignal-protocol-java) and [libsignal-metadata-java](https://github.com/signalapp/libsignal-metadata-java).
* signal-crypto: Cryptographic primitives such as AES-GCM. We use [RustCrypto](https://github.com/RustCrypto)'s where we can
  but sometimes have differing needs.
* device-transfer: Support logic for Signal's device-to-device transfer feature.
* attest: Functionality for remote attestation of [SGX enclaves](https://www.intel.com/content/www/us/en/architecture-and-technology/software-guard-extensions.html) and server-side [HSMs](https://en.wikipedia.org/wiki/Hardware_security_module).
...
* zkcredential: An abstraction for the sort of zero-knowledge credentials used by zkgroup, based on the paper "[The Signal Private Group System](https://eprint.iacr.org/2019/1416.pdf)" by Chase, Perrin, and Zaverucha.
...
* account-keys: Functionality for consistently using [PINs](https://signal.org/blog/signal-pins/) as passwords in Signal's Secure Value Recovery system, as well as other account-wide key operations.
* usernames: Functionality for username generation, hashing, and proofs.
* media: Utilities for manipulating media.

This repository is used by the Signal client apps ([Android](https://github.com/signalapp/Signal-Android), [iOS](https://github.com/signalapp/Signal-iOS), and [Desktop](https://github.com/signalapp/Signal-Desktop)) as well as
...
Our officially supported JDK version for Android builds is JDK 21, so be sure to install e.g. OpenJDK 21, and then point JAVA\_HOME to it.
...
contain native libraries for Debian-flavored x86\_64 Linux as well as Windows (x86\_64) and macOS
(x86\_64 and arm64). libsignal-android contains native libraries for armeabi-v7a, arm64-v8a, x86, and
x86\_64 Android. These are located in a Maven repository at
