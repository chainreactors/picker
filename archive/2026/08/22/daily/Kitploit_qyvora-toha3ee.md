---
title: qyvora-toha3ee
url: https://kitploit.com/en/tools/github/qyvora/qyvora-toha3ee
source: Kitploit
date: 2026-08-22
fetch_date: 2026-08-23T02:57:10.897079
---

# qyvora-toha3ee

[Skip to content](#main-content)

[![Kitploit](/_next/image?url=%2Flogo.png&w=64&q=75)KITPLOIT](/en)[Tools](/en/tools)[Blog](/en/blog)Categories

EN

[Submit](/en/submit)

[Tools](/en/tools)[Blog](/en/blog)Categories

[Submit](/en/submit)

EN

Hacking, PenTest, and Cybersecurity Tools for Your Security Arsenal!

Kitploit is a directory of hacking, cybersecurity, and pentesting tools. Discover the latest project updates to find vulnerabilities, analyze systems, automate testing, and strengthen your security.

·Analytics preferences·[Feeds](/en/feeds)·[Contact](/en/contact)·[Privacy](/en/privacy)·© 2026 Kitploit

Tool Directory

## Categories

[View all categories](/en/categories)

Loading categories

qyvora-toha3ee — Go-based network exploitation and MITM framework for authorized penetration testing, network reconnaissance, traffic interception, wireless security testing, and Layer 2/3/7 attack research. | Kitploit

[Tools](/en/tools)/![GitHub](/providers/github.png)GitHub/qyvora/qyvora-toha3ee

![](https://assets.kitploit.com/production/public/tools/50676/601c55014f43a77cadbcd18c090734d673f2d158027b7f47b76e74d996c090a4-display-v1.webp)

[OSINT (Open Source Intelligence)](/en/categories/osint)[Penetration Testing Frameworks](/en/categories/penetration-testing-frameworks)[Reconnaissance](/en/categories/reconnaissance)[Password Attacks](/en/categories/password-attacks)[Web Proxies & Interception](/en/categories/web-proxies-interception)[Exploitation](/en/categories/exploitation)[Post-Exploitation](/en/categories/post-exploitation)[Phishing](/en/categories/phishing)[Web Security](/en/categories/web-security)[Network Security](/en/categories/network-security)[Wireless Security](/en/categories/wireless-security)[Red Teaming](/en/categories/red-teaming)

31 day ago![Not yet reviewed](/_next/image?url=%2Fbadges%2Fkitploit_badge_not_reviewed_full.png&w=48&q=75)

### Most Popular

[View all →](/en/tools)

Discover the most used tools by our community.

Last 7 DaysLast 30 Days

Explore all tools

Browse our collection of tools

[View all tools →](/en/tools)

Share

![GitHub](/providers/github.png)qyvora/qyvora-toha3ee

# qyvora-toha3ee

Go-based network exploitation and MITM framework for authorized penetration testing, network reconnaissance, traffic interception, wireless security testing, and Layer 2/3/7 attack research.

[View Repository](https://github.com/qyvora/qyvora-toha3ee)

# toha3ee

![Build](https://img.shields.io/github/actions/workflow/status/qyvora/qyvora-toha3ee/ci.yml?branch=main&label=CI)
![Go Version](https://img.shields.io/github/go-mod/go-version/qyvora/qyvora-toha3ee)
![License](https://img.shields.io/github/license/qyvora/qyvora-toha3ee)
![Release](https://img.shields.io/github/v/release/qyvora/qyvora-toha3ee?sort=semver)
[![Documentation](https://img.shields.io/badge/docs-docs/-blue)](/docs)

A local & network security assessment framework written in Go. It is a
research and authorised-penetration-testing tool that covers the network
assessment pipeline — host and service discovery, enumeration, credential
auditing, vulnerability identification and authorised exploitation. Its ten
module categories span recon, enumeration, OSINT, MITM (ARP/DHCP/DNS/IPv6
poisoning and inline HTTP/HTTPS interception), wireless, switch-layer,
web, auth, espionage and post-exploitation — all driven from an interactive
REPL, a guided wizard, or one-shot command sequences.

MITM is one capability within this broader domain: TOHA3EE is not limited to
man-in-the-middle scenarios.

> **WARNING**: toha3ee actively redirects, poisons, decrypts and intercepts
> network traffic. Use it **only on networks you own or are explicitly
> authorised to test**. Running these modules against third parties is illegal
> in most jurisdictions. Read [`docs/security.md`](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/docs/security.md) first.

## Documentation

* **User** — [Getting started](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/docs/getting-started.md) · [User guide](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/docs/user-guide.md) · [Scripting](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/docs/scripting.md) · [Configuration](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/docs/configuration.md) · [FAQ](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/docs/faq.md)
* **Reference** — [Module reference](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/docs/module-reference.md) (all 73 modules) · [Reporting](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/docs/reporting.md)
* **Developer** — [Architecture](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/docs/architecture.md) · [Contributing](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/docs/contributing.md) · [Changelog](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/CHANGELOG.md)
* **Governance** — [Security](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/SECURITY.md) · [Code of Conduct](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/CODE_OF_CONDUCT.md) · [License](https://github.com/qyvora/qyvora-toha3ee/blob/HEAD/LICENSE)

## Install

One-liner installers fetch the prebuilt binary for your platform from the
latest release, verify its SHA-256 checksum and add it to your PATH. If no
prebuilt binary exists yet they build from source instead.

Linux / macOS:

root@kitploit:~

```
curl -fsSL https://raw.githubusercontent.com/qyvora/qyvora-toha3ee/main/scripts/install.sh | sh
```

Windows (PowerShell):

root@kitploit:~

```
irm https://raw.githubusercontent.com/qyvora/qyvora-toha3ee/main/scripts/install.ps1 | iex
```

Or from a checkout:

root@kitploit:~

```
make install   # installs ~/.local/bin/toha3ee and adds it to PATH
```

Install options (Unix): `--prefix <dir>` (default: `/usr/local/bin` as root,
else `~/.local/bin`), `--no-path` to skip editing your shell rc, `--from-source`
to build instead of downloading, and `TOHA3EE_VERSION=<tag>` to pin a release.
Run with `sudo sh ...` to install system-wide. The Windows installer puts the
binary in `%LOCALAPPDATA%\Programs\toha3ee\bin` and updates your user PATH;
Windows-on-ARM64 runs the x64 build.

On Linux the installer also registers the app with the desktop environment: it
installs the logo to the hicolor icon theme and drops a `.desktop` entry
next to the install prefix (e.g. `/usr/local/share` or `~/.local/share`), so
`toha3ee` shows up in GNOME's search with its icon. On Windows it copies the
`.ico` and creates a Start Menu shortcut. The release tarball/zip carry the
icon so the installer can register it from the same verified artifact.

Uninstall: delete the binary and the PATH line the installer added to your
shell rc (or `%LOCALAPPDATA%\Programs\toha3ee` on Windows).

## Build

Requires Go 1.26+ and libpcap.

root@kitploit:~

```
# Debian/Ubuntu
sudo apt install libpcap-dev

# then
go build ./cmd/toha3ee
```

Linux builds need libpcap headers (the installer's from-source fallback checks
for them and prints the right apt/dnf command if they are missing). macOS ships
libpcap with Xcode Command Line Tools.

## Quick start

root@kitploit:~

```
# Interactive console (bare command drops straight in)
sudo ./toha3ee --iface eth0

# Interactive console (explicit subcommand)
sudo ./toha3ee interactive --iface eth0

# Guided wizard
sudo ./toha3ee wizard --iface eth0

# One-shot: scan the subnet, then show what was found
sudo ./toha3ee --eval "net.scan; net.show" --iface eth0

# Non-interactive caplet script
sudo ./toha3ee run --iface eth0 caplets/basic.cap

# Dry-run a .toha3ee script (validates it, prints the plan, sends no packets)
./toha3ee --no-sudo build scripts/full-pipeline.toha3ee

# Execute a .toha3ee script non-interactively
sudo ./toha3ee script --iface eth0 scripts/full-pipeline.toha3ee
```

Most attack modules require root (raw sockets, packet capture and IP
forwarding). Run as root or with `CAP_NET_ADMIN`/`CAP_NET_RAW` where possible.
Add `--no-color` to disable colored output, `-v` for verbose logging.

The too...