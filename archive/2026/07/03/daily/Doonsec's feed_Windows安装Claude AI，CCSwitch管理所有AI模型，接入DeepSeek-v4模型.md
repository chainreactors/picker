---
title: Windows安装Claude AI，CCSwitch管理所有AI模型，接入DeepSeek-v4模型
url: https://mp.weixin.qq.com/s/fTau0KAW4ZtU4JMaAMejYg
source: Doonsec's feed
date: 2026-07-03
fetch_date: 2026-07-04T05:38:22.261739
---

# Windows安装Claude AI，CCSwitch管理所有AI模型，接入DeepSeek-v4模型
...
如果你也在使用多个 AI 大模型，比如 Claude、GPT 或一些本地模型，很可能遇到过这些头疼的问题：

不同平台来回切换、API 配置繁琐、开发环境互相割裂……连最基本的模型选择都变得特别低效。

最近有个话题很热：在本地安装 Claude Code，然后接入 DeepSeek‑V4 模型。
很多朋友会好奇：既然 Claude Code 已经自带了模型，为什么还要额外接 DeepSeek‑V4？

这其实是一种很聪明的搭配。Claude Code 是一个强大的命令行 AI 编码工具，而且它支持灵活接入不同的模型后端。
DeepSeek‑V4 作为新一代开源大模型，代码能力提升明显，更重要的是，它在国内有非常稳定流畅的官方 API 服务。
用这种方式，我们既能继续享受 Claude Code 的顺滑开发体验，又能借助 DeepSeek‑V4 的硬实力，同时不用为复杂的支付流程、帐号稳定性或网络波动而分心。对日常开发来说，这既高效又省心。

当然，想要把这套组合顺畅地跑起来，我们需要解决两个核心问题：一是让 Claude Code 顺利调用 DeepSeek‑V4 的接口，二是在不同模型之间统一调度，避免再次掉进“多平台切换”的坑。这里就需要用到一个轻量的小工具 —— CCSwitch。

本期内容，我会带你一步步完成三件事：

1. 配置基础运行环境，确保后续工具能正常安装。
2. 安装并配置 CCSwitch 和 Claude Code，实现模型的无缝接入。
3. 在实际开发场景里调用不同大模型，跑通完整的使用闭环。
