---
title: 【技术干货】带你了解爆火的 GraphQL（附带 Go 示例）
url: https://mp.weixin.qq.com/s?__biz=Mzg5NjEyMjA5OQ==&mid=2247496882&idx=1&sn=d49a6a994c17fb297783c32daff9cdcf&chksm=c007592ef770d03843ea4248353ba2c28473b9483fd1a450118f442fed13f41d3433d70543fa&scene=58&subscene=0#rd
source: 星阑科技
date: 2023-01-12
fetch_date: 2025-10-04T03:40:11.147809
---

# 【技术干货】带你了解爆火的 GraphQL（附带 Go 示例）

![cover_image](https://mmbiz.qpic.cn/mmbiz_jpg/wfFYMXc5G1PZLfpia3FgjOcRYtAGTOSIvHIjf5Ua30VLa2IRSMiaxReXNTNuwcRnlb04bib00WTriaxpmZgMRus78A/0?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/wfFYMXc5G1PZLfpia3FgjOcRYtAGTOSIv6UAlY5Av1iaGSEYhibAb1913cSfQicBkN22UVM3JKPqMA6Dmvbic80tEiaw/640?wx_fmt=png)

随后，Twitter 员工对其进行反击，要求马斯克学习 GraphQL 的工作原理。

![](https://mmbiz.qpic.cn/mmbiz_png/wfFYMXc5G1PZLfpia3FgjOcRYtAGTOSIvvGLMGRBfWPOZ02adSlpRFsqXXlYgfh5s3KevwrWSRGw5HUlgf72FxQ/640?wx_fmt=png)

> 新闻源地址：https://view.inews.qq.com/a/20221114A041F400

那么，究竟孰是孰非？下面是作者整理的架构图。

![](https://mmbiz.qpic.cn/mmbiz_png/wfFYMXc5G1PZLfpia3FgjOcRYtAGTOSIvKiaoPuSLUDYIqLtFShYkCticd0INfouyVZKduLOp1uCspIPLPsh443Sg/640?wx_fmt=png)
...
![](https://mmbiz.qpic.cn/mmbiz_png/wfFYMXc5G1PZLfpia3FgjOcRYtAGTOSIvJovUAibSsA8ia6SEibByl2iaT5KXXl0NN7hOVdRicWibDD6lwLLkfibCicAe1Q/640?wx_fmt=png)
...
```
% tree ..├── go.mod├── go.sum├── gql_type│   ├── mutation_type.go│   ├── post.go│   ├── post_type.go│   ├── query_type.go│   ├── user.go│   └── user_type.go├── main.go└── schema.gql
...
package gql_type
import (  "errors"  "sync")
type Post struct {  ID     int  UserID int  Title  string  Body   string}
var postMtx sync.RWMutexvar posts = make(map[int]*Post)var postID = 0
func InsertPost(post *Post) {  postMtx.Lock()  defer postMtx.Unlock()  postID += 1  post.ID = postID  posts[post.ID] = post}
func RemovePostByID(id int) {  postMtx.Lock()  defer postMtx.Unlock()  delete(posts, id)}
func GetPostByID(id int) (*Post, error) {  postMtx.RLock()  defer postMtx.RUnlock()  post, found := posts[id]  if !found {    return nil, errors.New("not found")  }  return post, nil}
func GetPostsForUser(userID int) []*Post {  postMtx.RLock()  defer postMtx.RUnlock()
  var res []*Post  for _, v := range posts {    if v.UserID == userID {      res = append(res, v)    }  }  return res}
```
...
var PostType = graphql.NewObject(graphql.ObjectConfig{  Name: "Post",  Fields: graphql.Fields{    "id": &graphql.Field{      Type: graphql.NewNonNull(graphql.ID),      Resolve: func(p graphql.ResolveParams) (interface{}, error) {        if post, ok := p.Source.(*Post); ok {          return post.ID, nil        }        return nil, nil      },    },    "title": &graphql.Field{      Type: graphql.NewNonNull(graphql.String),      Resolve: func(p graphql.ResolveParams) (interface{}, error) {        if post, ok := p.Source.(*Post); ok {          return post.Title, nil        }        return nil, nil      },    },    "body": &graphql.Field{      Type: graphql.NewNonNull(graphql.ID),      Resolve: func(p graphql.ResolveParams) ...