---
title: 【人工智能】微调的秘密武器：释放大模型的无限潜能
url: https://blog.csdn.net/nokiaguy/article/details/148468225
source: 一个被知识诅咒的人
date: 2025-06-07
fetch_date: 2025-10-06T22:53:54.578121
---
...
本文链接：<https://blog.csdn.net/nokiaguy/article/details/148468225>
...
L p r e t r a i n = − ∑ i log ⁡ P ( w i ∣ w 1 : i − 1 ; θ ) L\_{pretrain} = -\sum\_{i} \log P(w\_i | w\_{1:i-1}; \theta) Lpretrain​=−i∑​logP(wi​∣w1:i−1​;θ)

其中， w i w\_i wi​ 是词序列， θ \theta θ 是模型参数。微调的目标则是优化任务特定的损失函数，例如分类任务的交叉熵损失：

L f i n e t u n e = − ∑ i y i log ⁡ y ^ i L\_{finetune} = -\sum\_{i} y\_i \log \hat{y}\_i Lfinetune​=−i∑​yi​logy^​i​

其中， y i y\_i yi​ 是真实标签， y ^ i \hat{y}\_i y^​i​ 是模型预测。

#### 2.2 灾难性遗忘与正则化

微调可能导致模型遗忘预训练阶段学到的通用知识，即灾难性遗忘。为缓解这一问题，常用的正则化方法包括权重衰减和Dropout。权重衰减的损失函数为：

L = L t a s k + λ ∥ θ ∥ 2 2 L = L\_{task} + \lambda \|\theta\|\_2^2 L=Ltask​+λ∥θ∥
...
[在2025年，人工智能技术迅猛发展，具身智能（Embodied In...