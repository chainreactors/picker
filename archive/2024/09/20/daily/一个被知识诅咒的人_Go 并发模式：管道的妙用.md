---
title: Go 并发模式：管道的妙用
url: https://blog.csdn.net/nokiaguy/article/details/142357387
source: 一个被知识诅咒的人
date: 2024-09-20
fetch_date: 2025-10-06T18:25:22.688159
---
...
于 2024-09-19 12:49:58 首次发布

版权声明：本文为博主原创文章，遵循 [CC 4.0 BY-SA](http://creativecommons.org/licenses/by-sa/4.0/) 版权协议，转载请附上原文出处链接和本声明。

本文链接：<https://blog.csdn.net/nokiaguy/article/details/142357387>
...
**管道**（Pipeline）是构建系统抽象的另一种工具，尤其适用于需要处理数据流或批量数据的程序。管道这个词最早可以追溯到1856年，指的是用于输送液体的管道。在计算机科学中，我们也借用了这个概念，因为我们同样在“输送”数据。从概念上讲，管道就是一系列操作步骤，每个步骤都接收数据，进行处理，然后输出数据。我们称这些操作步骤为管道的**阶段**（stage）。

通过使用管道，你可以将每个阶段的任务分离开来，这样带来了许多好处。你可以独立修改每个阶段，不影响其他部分；可以自由组合这些阶段；可以让各个阶段并发处理，甚至可以对管道中的部分进行**扩展**（fan-out）或**限速**（rate-limit）。本文将通过简单的例子，展示如何构建管道的各个阶段，并引导你理解更多的并发模式。

#### 构建简单的管道阶段

管道阶段其实就是一个接收数据、对其进行处理、再输出数据的
...
08-28
![](https://csdnimg.cn/release/blogv2/dist/pc/img/re...