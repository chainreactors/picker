---
title: 安天AVL SDK反病毒引擎升级通告（20260523）
url: https://mp.weixin.qq.com/s/otdXOWoOvisIWQbSQI4wFg
source: Doonsec's feed
date: 2026-05-23
fetch_date: 2026-05-24T05:59:15.170059
---
...
统计周期：2026年05月16日~2026年05月22日

安天AVL SDK反病毒引擎本周共发布病毒库更新84次，日均更新12次，新增可检测恶意代码家族52个，新增可检测恶意代码变种6,680个，新增检测规则28,571条。
...
| --- | --- | --- |
| **序号** | **病毒名** | **病毒描述** |
| 1 | Trojan/MSIL.MiniPlasma | 该家族是一种木马病毒，主要利用系统漏洞进行传播和感染，该病毒通过伪装成正常软件或隐藏在破解程序、盗版软件中诱骗用户下载执行，窃取用户的敏感信息，包括银行账户、密码、个人身份信息等重要数据。 |
| 2 | Trojan/Win32.SepSys[Ransom] | 该家族是一种木马病毒，通常通过钓鱼邮件、恶意下载或漏洞利用等方式传播。一旦感染，该恶意软件会加密用户的文件，并要求支付赎金以获取解密密钥，对计算机系统造成严重威胁。 |
| 3 | Trojan/Win32.Cython[Dropper] | 该家族是一种木马病毒，主要通过网络下载方式进行传播，具有独立传播、潜藏性强等特点，会尝试隐藏自己，窃取用户的敏感信息，损害系统安全和稳定性。 |
| 4 | Trojan/Win32.Megamz | 该家族是一种木马病毒，主要通过钓鱼邮件、恶意网站或捆绑在破解软件中进行传播，一旦感染系统，会建立持久化并连接远程服务器下载执行恶意程序，严重威胁数据与系统安全。 |
| 5 | Trojan/Win32.BUTTSniff | 该家族是一种木马病毒，通过欺骗用户或携带其他恶意软件来进入受害者的计算机系统。一旦感染，会在后台运行并以隐藏的方式窃取敏感信息，监视用户的活动并向远程服务器发送数据。 |
...
截至2026年05月22日24:00，AVL SDK反病毒引擎可检出分布在8个基础分类，57,892个恶意代码家族的18,579,352个恶意代码变种、总检测规则数41,950,955条。
...
| 感染式病毒 | 59,636 | 6,813,894 |
| 蠕虫 | 315,183 | 3,943,303 |
| 木马 | 13,524,634 | 25,592,434 |
| 黑客工具 | 457,955 | 364,287 |
| 风险工具 | 1,206,343 | 2,226,494 |
| 流氓软件 | 3,015,564 | 3,009,362 |
| 垃圾文件 | 11 | 1,080 |
| 测试程序（自检用） | 26 | 101 |
| 合计 | 18,579,352 | 41,950,955 |
...
近一周监测到游蛇（银狐）黑产组织活动主要通过仿冒网站进行传播，恶意安装包程序，仿冒正规软件官网（如Todesk、搜狗输入法、WPS、谷歌浏览器）等。

其主要攻击手法为：

1、恶意安装包会在特定目录下创建多层随机命名的目录，释放恶意文件，并对关键目录设置严格的NTFS权限，使其在Windows资源管理器中完全隐藏，需命令行才可查看。

2、注册一个随机字符串命名的Windows服务实现开机自启，该服务启动后迅速显示为“已停止”状态，极具隐蔽性。

3、采用Module Stomping（模块覆写）技术，将Shellcode注入到系统进程sihost.exe中。

4、被注入的进程会连接到C2服务器22端口（伪装成SSH流量），全程使用加密的自定义协议。
...
[安天AVL SDK反病毒引擎升级通告（20260516）](https://mp.weixin.qq.com/s?__biz=MjM5MTA3Nzk4MQ==&mid=2650214753&idx=2&sn=823adda97260f5bbc6b641f77d18c39b&scene=21#wechat_redirect)

[安天AVL SDK反病毒引擎升级通告（20260509）](https://mp.weixin.qq.com/s?__biz=MjM5MTA3Nzk4MQ==&mid=2650214707&idx=2&sn=d0adbb466415a3ff47d78877ae4c4b39&scene=21#wechat_redirect)

[安天AVL SDK反病毒引擎升级通告（20260502）](https://mp.weixin.qq.com/s?__biz=MjM5MTA3Nzk4MQ==&mid=2650214677&idx=1&sn=6d7ed1cfe813675b2df2a726c1c553a5&scene=21#wechat_redirect)

[安天AVL SDK反病毒引擎升级通告（20260425）](https://mp.weixin.qq.com/s?__biz=MjM5MTA3Nzk4MQ==&mid=2650214550&idx=1&sn=8cb157259988cd0461532265448feba2&scene=21#wechat_redirect)

[安天AVL SDK反病毒引擎升级通告（20260418）](https://mp.weixin.qq.com/s?__biz=MjM5MTA3Nzk4MQ==&mid=2650214504&idx=1&sn=00cafd71b3ac59c42ae47e2ce2327883&scene=21#wechat_redirect)
