---
title: .NET 反混淆神器 de4dot 的可视化版本工具
url: https://mp.weixin.qq.com/s?__biz=MzUyOTc3NTQ5MA==&mid=2247496801&idx=1&sn=2caa22fc6829c41af632f774987aca1a&chksm=fa595a8ccd2ed39ae81570ae5dc4ac51b1c2a0d34e8669f015b818ac2974b15cab95c2f64ea2&scene=58&subscene=0#rd
source: dotNet安全矩阵
date: 2024-11-21
fetch_date: 2025-10-06T19:16:12.115077
---

# .NET 反混淆神器 de4dot 的可视化版本工具

![cover_image](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1Y9icZeicyl926NJyiaLSB72riaqMd78KGVuJFac1OC5KEDGlnVJogSBuWTRweppmwgdJiczgSC87K2tl0Q/0?wx_fmt=jpeg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1Y9icZeicyl926NJyiaLSB72riaq5jeprj5pk2sJn5ia9XpBziaGK00XpiaEKEDdKja7byMpDGA73ROXGzAbw/640?wx_fmt=jpeg&from=appmsg)

03

使用方法

使用前，确保工具目录中包含默认的 de4dot.exe ，该版本基于.NET 4.5编译，或通过设置手动指定 de4dot 的路径。

![](https://mmbiz.qpic.cn/mmbiz_png/NO8Q9ApS1Y9icZeicyl926NJyiaLSB72riaqmxE9Nbark9rgpK2lnVibT2mDSFOHQjic5MBG1u5f9xFWWfBDTOCMibNEw/640?wx_fmt=png&from=appmsg)

随后，用户只需将目标文件拖放到界面，即可自动加载，并且支持强制识别混淆器、选择特定混淆方法，以及控制反混淆行为，比如禁止重命名。如下图所示。

![](https://mmbiz.qpic.cn/mmbiz_png/NO8Q9ApS1Y9icZeicyl926NJyiaLSB72riaqASoLlaqbKhvibLtdFdNRwH9PSOATCRtfkj2h7OKzJYF5icUATT9YSyfg/640?wx_fmt=png&from=appmsg)

04

原理解析

Sharp4de4dot.exe 程序在启动时会自动检测 de4dot.exe 的路径，如果需要手动调整路径，可以在设置界面中修改相关字段，具体代码如下所示。

```
```
if (Properties.Settings.Default.deobfPath.Length <= 0)
    Properties.Settings.Default.deobfPath = Application.StartupPath + @"\de4dot45\de4dot.exe";

if (Properties.Settings.Default.deobfPath64.Length <= 0)
    Properties.Settings.Default.deobfPath64 = Application.StartupPath + @"\de4dot45\de4dot64.exe";
```
```

将混淆的 .NET 程序文件（如 .dll 或 .exe）直接拖放到主界面，工具会自动加载并准备处理，接着，根据需求配置反混淆选项，再点击处理按钮，工具将调用 Processor 类执行反混淆操作。
...
工具会根据用户设置自动调整处理流程，并生成反混淆后的文件。工具已经打包在星球，感兴趣的朋友可以加入自取。
