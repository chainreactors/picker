---
title: Bkcrack - Crack Legacy Zip Encryption With Biham And Kocher's Known Plaintext Attack
url: https://buaq.net/go-145426.html
source: unSafe.sh - 不安全
date: 2023-01-14
fetch_date: 2025-10-04T03:51:15.253321
---
...
![](https://8aqnet.cdn.bcebos.com/a084ffcab88f46f5853ecdbbdfc39f44.jpg)

Bkcrack - Crack Legacy Zip Encryption With Biham And Kocher's Known Plaintext Attack

Crack legacy zip encryption with Biham and Kocher's known plaintext attack. Overview A ZIP
*2023-1-13 19:30:0
Author: [www.kitploit.com(查看原文)](/jump-145426.htm)
阅读量:38
收藏*

---

[![](https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjjC3XSQvb60FabL6LsNZJ7951VKHxdjHrUCnwgGV7QwqfPr70etJD2NJgesotKmqE1Sblur-11xfLZ4zaCi8vFQ36TbJMFiWe2JVG2p20r5Tw0CIH7Y7gFafGumYaTimHAU6WPk9yisW5XeTZimeqfso4Ylgw2tAmgxklGFHRbp4qLXFxMP6bUU66a5A/w640-h248/bkcrack.png)](https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjjC3XSQvb60FabL6LsNZJ7951VKHxdjHrUCnwgGV7QwqfPr70etJD2NJgesotKmqE1Sblur-11xfLZ4zaCi8vFQ36TbJMFiWe2JVG2p20r5Tw0CIH7Y7gFafGumYaTimHAU6WPk9yisW5XeTZimeqfso4Ylgw2tAmgxklGFHRbp4qLXFxMP6bUU66a5A/s891/bkcrack.png)

Crack legacy zip encryption with Biham and Kocher's known plaintext attack.

## Overview

A ZIP archive may contain many entries whose content can be compressed and/or encrypted. In particular, entries can be encrypted with a password-based [Encryption](https://www.kitploit.com/search/label/Encryption "Encryption") Algorithm symmetric encryption algorithm referred to as traditional PKWARE encryption, legacy encryption or ZipCrypto. This algorithm generates a pseudo-random stream of bytes (keystream) which is XORed to the entry's content (plaintext) to produce encrypted data (ciphertext). The generator's state, made of three 32-bits integers, is initialized using the password and then continuously updated with plaintext as encryption goes on. This encryption algorithm is vulnerable to known plaintext attacks as shown by Eli Biham and Paul C. Kocher in the research paper [A known plaintext attack on the PKZIP stream cipher](https://doi.org/10.1007/3-540-60590-8_12 "A known plaintext attack on the PKZIP stream cipher"). Given ciphertext and 12 or more bytes of the corresponding plaintext, the internal state of the keystream generator can be recovered. This internal state is enough to decipher ciphertext entirely as well as other entries which were encrypted with the same password. It can also be used to [bruteforce](https://www.kitploit.com/search/label/Bruteforce "bruteforce") the password with a complexity of *nl-6* where *n* is the size of the character set and *l* is the length of the password.
...
You can get the latest official release on [GitHub](https://github.com/kimci86/bkcrack/releases "GitHub").
...
Alternatively, you can compile the project with [CMake](https://cmake.org "CMake").
...
bkcrack is available in the package [repositories](https://www.kitploit.com/search/label/Repositories "repositories") listed on the right. Those packages are provided by external maintainers.
...
The deciphered data might be compressed depending on whether [compression](https://www.kitploit.com/search/label/Compression "compression") was used or not when the zip file was created. If deflate compression was used, a Python 3 script provided in the `tools` folder may be used to decompress data.
...
Do not hesitate to suggest improvements or submit pull requests on [GitHub](https://github.com/kimci86/bkcrack "GitHub").

## License

This project is provided under the terms of the [zlib/png license](https://opensource.org/licenses/Zlib "zlib/png license").

Bkcrack - Crack Legacy Zip Encryption With Biham And Kocher's Known Plaintext Attack
![Bkcrack - Crack Legacy Zip Encryption With Biham And Kocher's Known Plaintext Attack](https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjjC3XSQvb60FabL6LsNZJ7951VKHxdjHrUCnwgGV7QwqfPr70etJD2NJgesotKmqE1Sblur-11xfL...