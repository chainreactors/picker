---
title: 【人工智能】大模型的成长日记：从训练到应用的全面蜕变
url: https://blog.csdn.net/nokiaguy/article/details/148004157
source: 一个被知识诅咒的人
date: 2025-05-17
fetch_date: 2025-10-06T22:26:39.086929
---
...
2. 大模型的训练：从零到英雄
    ...
    数学上，语言建模的损失函数为交叉熵损失：
     L = − ∑ t = 1 T log ⁡ P ( w t ∣ w 1 : t − 1 ; θ ) L = -\sum\_{t=1}^T \log P(w\_t | w\_{1:t-1}; \theta) L=−t=1∑T​logP(wt​∣w1:t−1​;θ)
    其中：

( w\_t )：序列中的第 ( t ) 个词
 ( P(w\_t | w\_{1:t-1}; \theta) )：模型预测的下一个词的概率
 ( T )：序列长度
 ( \theta )：模型参数
...
class SimpleTransformer(nn.Module):
 def **init**(self, vocab\_size, d\_model, nhead, num\_layers):
 super(SimpleTransformer, self).**init**()
 self.embedding = nn.Embedding(vocab\_size, d\_model)
 self.transformer = nn.TransformerEncoder(
 nn.TransformerEncoderLayer(d\_model, n
