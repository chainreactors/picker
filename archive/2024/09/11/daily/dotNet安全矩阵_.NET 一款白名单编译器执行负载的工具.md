---
title: .NET 一款白名单编译器执行负载的工具
url: https://mp.weixin.qq.com/s?__biz=MzUyOTc3NTQ5MA==&mid=2247495187&idx=1&sn=59d96f252347111bca9114feb4c6e3c6&chksm=fa5940fecd2ec9e81b7a7806b179c909f33eb64d26ee7b5bf63b17a0153320fd0c5f16944b41&scene=58&subscene=0#rd
source: dotNet安全矩阵
date: 2024-09-11
fetch_date: 2025-10-06T18:29:08.554418
---

# .NET 一款白名单编译器执行负载的工具

![cover_image](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1Y8KnQ9tvBogsL9cibsMiajibhmZkNIHTO2uXBqzH4cmZwG1Ovrac7Jrx72smtfibgZ3L7Z0GV3CbOwIhg/0?wx_fmt=jpeg)
...
Sharp4fsi.exe是一款基于F#的交互式编译器执行攻击负载的工具，通过封装或修改原始编译器，能够执行包含恶意代码的F#脚本文件。由于F#的交互式编译器本身带有微软的签名，因此Sharp4fsi.exe也继承了这一特性，使得在执行时能够绕过一些基于签名验证的安全防护软件。

![](https://mmbiz.qpic.cn/mmbiz_jpg/NO8Q9ApS1Y8KnQ9tvBogsL9cibsMiajibhmvB0Y9BIC6pw9zVxDjUpCeSSJ4Otvj7gT7ibOhGvH1y5JwF8aZGkLaoA/640?wx_fmt=jpeg&from=appmsg)

03

使用方法

Sharp4fsi.exe的使用方法非常简单，只需在命令行中指定要执行的F#脚本文件即可。以下是一个基本的命令示例：

```
Sharp4fsi.exe Fsharp4Process.fsscript
```

在这个例子中，Fsharp4Process.fsscript是包含F#代码的脚本文件，工具将负责执行该文件中的代码。下面fsccript代码中执行了whoami命令，返回如下图所示

![](https://mmbiz.qpic.cn/mmbiz_png/NO8Q9ApS1Y8KnQ9tvBogsL9cibsMiajibhmq1mpQY26xf1AOkeMAKXuktiafSK8libhiciaIcmbqnTH3AUMibXwvI9L4Rg/640?wx_fmt=png&from=appmsg)

综上，Sharp4fsi.exe作为一款利用F#编译器执行攻击负载的工具，由于本身带有微软的签名，使得它在执行时能够绕过一些基于签名验证的安全防护软件。对现有的安全防护策略构成了挑战。工具已经打包在星球，感兴趣的朋友可以加入自取。
