...
source: 小刀志
date: 2026-09-01
fetch_date: 2026-09-02T06:39:42.370199
---
...
这里的 `CRLpoint` 在实现里会从证书扩展中解出来：

`CRL` 是 Certificate Revocation List，表示已经撤销的证书列表；`CRLpoint` 是证书扩展里给出的 CRL 分发地址，通常是一个或多个 URL。
...
BOOL CalculateCertCRLpoint(
    ...
    PCERT_EXTENSION pe = CertFindExtension(szOID_CRL_DIST_POINTS, cExtensions, rgExtensions);
    if (!pe) {
        return FALSE;
    }
    if (!CryptDecodeObject(MY_ENCODING, szOID_CRL_DIST_POINTS,
        pe->Value.pbData, pe->Value.cbData,
        CRYPT_DECODE_NOCOPY_FLAG | CRYP...