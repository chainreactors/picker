---
title: Attacco e-mail spoofing: capirlo per difendersi
url: https://www.cybersecurity360.it/nuove-minacce/attacco-e-mail-spoofing-capirlo-per-difendersi/
source: Over Security - Cybersecurity news aggregator
date: 2022-11-18
fetch_date: 2025-10-03T23:09:15.144597
---

# Attacco e-mail spoofing: capirlo per difendersi

[Vai al contenuto principale](#main-content)
[Vai al footer](#footer-content)

![logo](data:image/png;base64...)![logo](https://cdnd360.it/networkdigital360/nd360-neg.svg)

[I NOSTRI SERVIZI](https://www.cybersecurity360.it/about-network)

Menu

[![Vai alla homepage di CyberSecurity](data:image/png;base64...)![Vai alla homepage di CyberSecurity](https://dnewpydm90vfx.cloudfront.net/wp-content/uploads/2024/03/cybersecurity_logo-768x55.png)](https://www.cybersecurity360.it)

## Attacco e-mail spoofing: capirlo per difendersi

* [Cybersecurity Nazionale](https://www.cybersecurity360.it/cybersecurity-nazionale/)
* Malware e attacchi
  + [Malware e attacchi](https://www.cybersecurity360.it/nuove-minacce/)
  + [Ransomware](https://www.cybersecurity360.it/nuove-minacce/ransomware/)
* Norme e adeguamenti
  + [Norme e adeguamenti](https://www.cybersecurity360.it/legal/)
  + [Privacy e Dati personali](https://www.cybersecurity360.it/legal/privacy-dati-personali/)
* [Soluzioni aziendali](https://www.cybersecurity360.it/soluzioni-aziendali/)
* [Cultura cyber](https://www.cybersecurity360.it/cultura-cyber/)
* [News, attualità e analisi Cyber sicurezza e privacy](https://www.cybersecurity360.it/news/)
* [Corsi cybersecurity](https://www.cybersecurity360.it/corsi-cybersecurity/)
* [Chi siamo](https://www.cybersecurity360.it/about/)

* [![Vai alla homepage di CyberSecurity](data:image/png;base64...)![Vai alla homepage di CyberSecurity](https://dnewpydm90vfx.cloudfront.net/wp-content/uploads/2024/03/cybersecurity_neg_logo-768x55.png)](https://www.cybersecurity360.it)
* Seguici
* + [twitter](https://twitter.com/Cybersec360)
  + [linkedin](https://www.linkedin.com/company/cybersecurity360/)
  + [Newsletter](https://www.cybersecurity360.it/newsletter-signin/)
  + [Rss Feed](#rssModal)
  + [Chi siamo](https://www.cybersecurity360.it/about)
* AREA PREMIUM
* [Whitepaper](https://www.cybersecurity360.it/whitepaper/)
* [Eventi](https://www.cybersecurity360.it/eventi/)
* [Webinar](https://www.cybersecurity360.it/webinar/)
* CANALI
* [Cybersecurity nazionale](https://www.cybersecurity360.it/cybersecurity-nazionale/)
* [Malware e attacchi](https://www.cybersecurity360.it/nuove-minacce/)
* + [Ransomware](https://www.cybersecurity360.it/nuove-minacce/ransomware/)* [Norme e adeguamenti](https://www.cybersecurity360.it/legal/)
  * + [Privacy e Dati personali](https://www.cybersecurity360.it/legal/privacy-dati-personali/)* [Soluzioni aziendali](https://www.cybersecurity360.it/soluzioni-aziendali/)
    * [Cultura cyber](https://www.cybersecurity360.it/cultura-cyber/)
    * [L'esperto risponde](https://www.cybersecurity360.it/esperto-risponde/)
    * [News, attualità e analisi Cyber sicurezza e privacy](https://www.cybersecurity360.it/news/)
    * [Corsi cybersecurity](https://www.cybersecurity360.it/corsi-cybersecurity/)
    * [Chi siamo](https://www.cybersecurity360.it/about/)

[Cybersecurity nazionale](https://www.cybersecurity360.it/cybersecurity-nazionale/)
[Malware e attacchi](https://www.cybersecurity360.it/nuove-minacce/)
[Norme e adeguamenti](https://www.cybersecurity360.it/legal/)
[Soluzioni aziendali](https://www.cybersecurity360.it/soluzioni-aziendali/)
[Cultura cyber](https://www.cybersecurity360.it/cultura-cyber/)
[L'esperto risponde](https://www.cybersecurity360.it/esperto-risponde/)
[News, attualità e analisi Cyber sicurezza e privacy](https://www.cybersecurity360.it/news/)
[Corsi cybersecurity](https://www.cybersecurity360.it/corsi-cybersecurity/)
[Chi siamo](https://www.cybersecurity360.it/about/)

LA GUIDA COMPLETA

# Attacco e-mail spoofing: capirlo per difendersi

* [Home](https://www.cybersecurity360.it)
* [Malware e attacchi hacker](https://www.cybersecurity360.it/nuove-minacce/)

La maggior parte degli attacchi informatici utilizza la posta elettronica come vettore per andare a segno. Proviamo quindi a descrivere le tecniche usate dai criminal hacker in un attacco di e-mail spoofing per capirlo a fondo e organizzare al meglio le strategie di difesa

Pubblicato il 17 Nov 2022

[Alessandro Bertoldi](https://www.cybersecurity360.it/giornalista/alessandro-bertoldi/)

Bertoldi Cybersecurity

![Attacco e-mail spoofing la guida](data:image/png;base64...)![Attacco e-mail spoofing la guida](https://dnewpydm90vfx.cloudfront.net/wp-content/uploads/2019/05/Attacco-e-mail-spoofing.jpg)

*I passaggi illustrati nell’articolo sono stati redatti a solo scopo dimostrativo e sfruttano vulnerabilità note. Bertoldi Cybersecurity e i suoi collaboratori non hanno alcuna responsabilità per gli utilizzi impropri di quanto esposto.*

L’e-mail spoofing è una delle [tecniche criminali](https://www.cybersecurity360.it/nuove-minacce/attacchi-hacker-strumenti-e-tecniche-dei-nuovi-cyber-criminali/) più utilizzate per [veicolare malware](https://www.cybersecurity360.it/tag/malware/) di ogni genere e **si basa sulla falsificazione di diverse informazioni** come, ad esempio, l’identità di un host (computer o server connesso in rete) o il mittente di un messaggio.

Una volta che un attaccante riesce ad impersonare qualcun altro all’interno di una rete, gli è possibile intercettare informazioni riservate, diffonderne di false o effettuare qualsiasi tipo di attacco mirato ad ottenere l’accesso ad informazioni “riservate” e credenziali degli utenti.

Indice degli argomenti

* [La struttura di una e-mail](#La_struttura_di_una_e-mail)
* [Attacco e-mail spoofing: analisi del target e fase preparatoria](#Attacco_e-mail_spoofing_analisi_del_target_e_fase_preparatoria)
  + [Ricerca OSINT](#Ricerca_OSINT)
  + [Analisi del dominio ed e-mail scraping](#Analisi_del_dominio_ed_e-mail_scraping)
  + [Studio del criterio con i quali sono stati costruiti gli indirizzi e-mail](#Studio_del_criterio_con_i_quali_sono_stati_costruiti_gli_indirizzi_e-mail)
  + [Spedizione di una e-mail al target](#Spedizione_di_una_e-mail_al_target)
  + [Consultazione DNS di dominio](#Consultazione_DNS_di_dominio)
* [Attacco e-mail spoofing: i dettagli](#Attacco_e-mail_spoofing_i_dettagli)
  + [Falsificazione dell’header tramite client di posta](#Falsificazione_dellheader_tramite_client_di_posta)
* [Falsificazione dell’header tramite client di posta con attacco ASCII](#Falsificazione_dellheader_tramite_client_di_posta_con_attacco_ASCII)
  + [Sessione telnet sulla piattaforma di filtraggio della posta](#Sessione_telnet_sulla_piattaforma_di_filtraggio_della_posta)
* [Sessione telnet direttamente sul server di posta](#Sessione_telnet_direttamente_sul_server_di_posta)
* [Attacco email alle Whitelist](#Attacco_email_alle_Whitelist)
* [Attacco e-mail spoofing: le soluzioni per proteggersi](#Attacco_e-mail_spoofing_le_soluzioni_per_proteggersi)
  + [Sicurezza DNS: SPF, DKIM e DMARC](#Sicurezza_DNS_SPF_DKIM_e_DMARC)
  + [Sistema di filtraggio della posta elettronica](#Sistema_di_filtraggio_della_posta_elettronica)
  + [Anti-spoofing sul server di posta](#Anti-spoofing_sul_server_di_posta)
  + [Binding tra server di posta e sistema di filtraggio](#Binding_tra_server_di_posta_e_sistema_di_filtraggio)

## **La struttura di una e-mail**

Per capire come funziona un attacco e-mail spoofing è utile conoscere la struttura di una e-mail e comprendere come viene composto un messaggio di posta elettronica.

Una e-mail è composta da due parti:

* **header**: è formato dai campi mittente, destinatario, data e oggetto, di cui i primi tre sono obbligatori. Tutti questi elementi sono quelli visualizzati nel programma di gestione della posta elettronica (client desktop o webmail) e possono essere falsificati da chi spedisce, sia tramite il client di posta sia utilizzando una sessione telnet, come vedremo a breve.
* **envelope**: identifica con precisione il percorso delle e-mail attraverso i suoi passaggi, dall’indirizzo IP di invio all’indirizzo IP di ricezione.

## **Attacco e-mail spoofing: analisi del target e fase preparatoria**

Per portare a termine con successo un attacco di tipo e-mail spoofing, dobbiamo completare alcune fasi preparatorie necessari...